...
from rfid import excel, lessons
from tests.helpers import ROSTER_NAMES, active, make_roster_file, open_sheet
...
class TestReading:
    def test_group_from_title_row(self, roster_file):
        assert excel.read_roster(roster_file).group_name == "1000000/10001"

    def test_finds_all_students(self, roster_file):
        students = excel.read_roster(roster_file).students
    ...
    def test_note_below_list_is_not_a_student(self, roster_file):
        ...
        names = [s.full_name for s in excel.read_roster(roster_file).students]
        assert not any("Староста" in n for n in names)

    def test_rows_point_at_real_cells(self, roster_file):
        sheet = open_sheet(roster_file)
        for student in excel.read_roster(roster_file).students:
    ...
    def test_dates_start_right_after_names(self, roster_file):
        parsed = excel.read_roster(roster_file)
    ...
    def test_group_code_found_without_the_word(self, tmp_path):
        ...
        active(workbook)["A1"] = "ИКНК 1000000/10002"
        workbook.save(path)
        assert excel.read_roster(path).group_name == "1000000/10002"

    def test_plain_text_above_header_is_used(self, tmp_path):
        path = make_roster_file(tmp_path / "файл.xlsx")
        workbook = load_workbook(path)
        active(workbook)["A1"] = "Первый курс, поток А"
        workbook.save(path)
        assert excel.read_roster(path).group_name == "Первый курс, поток А"

    def test_group_falls_back_to_filename(self, tmp_path):
        path = tmp_path / "БИ-11.xlsx"
        make_roster_file(path, group="")
        workbook = load_workbook(path)
        active(workbook)["A1"] = None
        workbook.save(path)
        assert excel.read_roster(path).group_name == "БИ-11"

    def test_file_without_header_is_rejected(self, tmp_path):
        path = tmp_path / "мусор.xlsx"
        workbook = Workbook()
        active(workbook)["A1"] = "что-то не то"
        workbook.save(path)
        with pytest.raises(excel.RosterError):
            excel.read_roster(path)
...
class TestNameMatching:
    ...
    def test_unknown_name_reported(self, roster_file):
        parsed = excel.read_roster(roster_file)
        assert excel.unmatched_names(parsed, ["Образцова Проба Тестовна", "Кто-то Левый"]) == [
            "Кто-то Левый"
        ]
...
class TestWriting:
    def test_merged_name_does_not_block_attendance(self, tmp_path):
        ...
        excel.write_attendance(excel.read_roster(path), marks)
        ...
        assert [s.full_name for s in excel.read_roster(path).students] == names

    def test_late_threshold_includes_exactly_ten_minutes(self, roster_file):
        parsed = excel.read_roster(roster_file)
        excel.write_attendance(parsed, {DAY_ONE: {
    ...
    def test_other_lesson_the_same_day_is_not_late(self, tmp_path):
        ...
        for path in (morning, afternoon):
            assert open_sheet(path)["C3"].fill.patternType is None, path.name
    ...
    def test_late_to_a_shared_lesson_is_still_caught(self, tmp_path):
        ...
        assert open_sheet(first)["C3"].fill.patternType is None
        assert open_sheet(second)["C3"].fill.patternType == "solid"
    ...
    def test_reexport_clears_outdated_late_fill(self, roster_file):
        ...
        excel.write_attendance(parsed, {DAY_ONE: {NAMES[1]: datetime(2026, 9, 20, 9, 10)}})
        sheet = open_sheet(roster_file)
        assert sheet["C4"].fill.patternType is None
    ...
    def test_time_in_file_is_never_replaced_by_dash(self, roster_file):
        ...
        excel.write_attendance(parsed, {DAY_ONE: {NAMES[1]: datetime(2026, 9, 20, 9, 10)}})
        excel.write_attendance(parsed, {DAY_ONE: {}})
        assert open_sheet(roster_file)["C4"].value == "09:10"

    def test_date_column_added(self, roster_file):
        parsed = excel.read_roster(roster_file)
        excel.write_attendance(parsed, {DAY_ONE: {NAMES[0]: datetime(2026, 9, 20, 9, 2)}})
    ...
    def test_rewriting_same_date_reuses_column(self, roster_file):
        ...
        parsed = excel.read_roster(roster_file)
        excel.write_attendance(parsed, {DAY_ONE: {NAMES[0]: datetime(2026, 9, 20, 9, 2)}})
        excel.write_attendance(parsed, {DAY_ONE: {NAMES[1]: datetime(2026, 9, 20, 9, 7)}})
    ...
    def test_note_under_the_list_survives(self, roster_file):
        ...
        assert isinstance(note, str) and "Староста" in note
    ...
    def test_reparse_after_write_is_stable(self, roster_file):
        parsed = excel.read_roster(roster_file)
        excel.write_attendance(parsed, {DAY_ONE: {NAMES[0]: datetime(2026, 9, 20, 9, 2)}})
        again = excel.read_roster(roster_file)
...
class TestSubjectFolders:
    def test_folders_with_lists_are_subjects(self, tmp_path):
        ...
        assert [s.name for s in excel.discover_subjects(tmp_path)] == [
    ...
    def test_empty_folders_are_skipped(self, tmp_path):
        ...
        assert excel.discover_subjects(tmp_path) == []
    ...
    def test_only_subfolders_are_returned(self, tmp_path):
        ...
        assert [f.name for f in excel.discover_subjects(tmp_path)] == ["Физика"]
    ...
    def test_rosters_inside_are_listed(self, tmp_path):
        ...
        subject = excel.discover_subjects(tmp_path)[0]
    ...
    def test_excel_lock_files_ignored(self, tmp_path):
        ...
        assert excel.discover_subjects(tmp_path)[0].group_count == 1

    def test_missing_directory_is_not_an_error(self, tmp_path):
        assert excel.discover_subjects(tmp_path / "нет") == []

    def test_as_folder_takes_any_directory(self, tmp_path):
        make_roster_file(tmp_path / "г.xlsx")
        folder = excel.as_folder(tmp_path)
...
class TestFullRoster:
    ...
    def test_parses(self, full_roster_file):
        parsed = excel.read_roster(full_roster_file)
    ...
    def test_note_not_counted(self, full_roster_file):
        names = [s.full_name for s in excel.read_roster(full_roster_file).students]
        assert not any("Староста" in n for n in names)
...
class TestSessionSplitting:
    ...
    def test_one_lesson_gives_one_start(self):
        ...
        assert lessons.session_starts(arrivals) == [self.at(9, 0)]

    def test_next_lesson_opens_a_new_session(self):
        arrivals = [self.at(9, 0), self.at(9, 5), self.at(14, 0), self.at(14, 3)]
        assert lessons.session_starts(arrivals) == [self.at(9, 0), self.at(14, 0)]
    ...
    def test_back_to_back_lessons_are_separated(self):
        ...
        assert lessons.session_starts(arrivals) == [self.at(9, 0), self.at(11, 0)]

    def test_exactly_one_cycle_later_is_a_new_lesson(self):
        """Пары идут ровно через цикл — сравнение обязано быть нестрогим."""
        assert lessons.session_starts([self.at(9, 0), self.at(11, 0)]) == [
    ...
    def test_tap_during_the_break_does_not_poison_the_next_lesson(self):
        ...
        starts = lessons.session_starts(arrivals)
        assert starts == [self.at(9, 0), self.at(11, 0)]
        assert lessons.is_late(self.at(11, 0), starts) is False
        assert lessons.is_late(self.at(11, 6), starts) is False

    def test_order_of_input_does_not_matter(self):
        jumbled = [self.at(14, 3), self.at(9, 0), self.at(14, 0), self.at(9, 5)]
        assert lessons.session_starts(jumbled) == [self.at(9, 0), self.at(14, 0)]

    def test_no_arrivals(self):
        assert lessons.session_starts([]) == []
    ...
    def test_latecomer_does_not_open_a_new_lesson(self):
        ...
        assert lessons.session_starts(arrivals) == [self.at(9, 0)]
        assert lessons.is_late(self.at(9, 40), lessons.session_starts(arrivals))
    ...
    def test_very_late_arrival_still_counts_as_late(self):
        ...
        assert lessons.session_starts(arrivals) == [self.at(9, 0)]
        assert lessons.is_late(self.at(10, 20), lessons.session_starts(arrivals))
    ...
    def test_cycle_is_longer_than_the_late_window(self):
        ...
        assert lessons.LESSON_CYCLE > lessons.LATE_AFTER

    def test_cycle_is_lesson_plus_break(self):
        assert lessons.LESSON_CYCLE == lessons.LESSON_LENGTH + lessons.BREAK_LENGTH

    def test_arrival_is_matched_to_its_own_lesson(self):
        starts = lessons.session_starts([self.at(9, 0), self.at(14, 0)])
        assert lessons.session_start_for(self.at(9, 30), starts) == self.at(9, 0)
        assert lessons.session_start_for(self.at(14, 30), starts) == self.at(14, 0)

    def test_arrival_before_any_lesson_has_no_start(self):
        starts = lessons.session_starts([self.at(9, 0)])
        assert lessons.session_start_for(self.at(8, 0), starts) is None
        assert lessons.is_late(self.at(8, 0), starts) is False

    def test_late_only_after_the_threshold(self):
        starts = lessons.session_starts([self.at(9, 0)])
        assert lessons.is_late(self.at(9, 0) + lessons.LATE_AFTER, starts) is True
        assert lessons.is_late(self.at(9, 9), starts) is False
...
class TestWriteSafety:
    ...
    def test_failed_write_leaves_the_roster_intact(self, roster_file, monkeypatch):
        ...
        with pytest.raises(PermissionError):
            excel.write_attendance(excel.read_roster(roster_file), {DAY_ONE: {}})
