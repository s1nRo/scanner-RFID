"""Командная строка.
    ...
    python -m rfid scan --enroll    отмечать и сразу привязывать новые карты
...
MENU = [
    ...
    ("2", "Отмечать и сразу привязывать новые карты", ["scan", "--enroll"]),
    ("3", "Привязать карты к студентам", ["enroll"]),
    ("4", "Кто пришёл сегодня", ["report"]),
    ("5", "Опоздавшие — показывают конспект", ["late"]),
    ("6", "Чья это карта", ["whois"]),
    ("7", "Предметы и группы", ["subjects"]),
    ("8", "Кому какая карта принадлежит", ["students", "list"]),
    ("9", "Заполнить файлы групп", ["export"]),
    # Из меню режим спрашивается, а не берётся обычный молча.
    ("10", "Обновить базу из файлов групп", ["import", "--choose"]),
    ("11", "Проверить оборудование", ["doctor"]),
]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    scan.add_argument("--enroll", action="store_true",
                      help="незнакомую карту сразу привязать к студенту из списка")
